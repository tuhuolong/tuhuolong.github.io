...
---
    ...
    谁不想知道虚拟机是怎样工作的？不过，比起自己写一个虚拟机，更好的办法是使用大公司的产品。在这篇文章中，我将介绍如何在你的程序中使用
    ...
    里，每个上下文是分开的，它们有自己的全局模型。一个上下文就是一个独立的执行环境，相互之间没有关联，
 ...
 do it! V8
    ...
    为了把这个类完全嵌入脚本中，我们需要映射类成员函数和类成员变量。第一步是在我们的上下文中映射一个类模型
    ...
    和普通访问器一样，我们必须处理参数。要访问我们的类，必须从内部域（第一个）中取得类指针。把内部域映射到
